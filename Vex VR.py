...
def generateRandomPoint():
    xCoordinate = random.randint(-4,8)*100
    yCoordinate = random.randint(-4,8)*100
...
def driveUsingDistanceSensor(setpoint,duration):
    ...
    while(brain.timer_time(SECONDS)<duration):
        ...
        myDistance = front_distance.get_distance(MM)


        error = myDistance - setpoint
        k = 1
        output_speed = k*error
        drivetrain.set_drive_velocity(output_speed, PERCENT)
        ...
        if(myDistance < setpoint):
            drivetrain.drive(REVERSE)
        elif (myDistance > setpoint):
            drivetrain.drive(FORWARD)
        else:
            drivetrain.stop()
...
def main():
    ...
    brain.print("target location is = ( " + str(target[0]) + " , " + str(target[1]) + " )" )
    pen.move(DOWN)
